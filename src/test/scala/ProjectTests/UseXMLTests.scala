...
class UseXMLTests extends AnyFunSuite {
  ...
  test("Same list when you write and load xml"){
    xmlTester.writeXMLData(itemList1)
    val tempItems: List[Item] = xmlTester.loadExistingFile().toList
